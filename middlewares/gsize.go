...
func (mbr *maxBytesReader) tooLarge() (n int, err error) {
	...
	if !mbr.wasAborted {
		...
		ctx.JSON(http.StatusRequestEntityTooLarge, gin.H{"msg": "request too large"})
...
// Size Size
func Size() gin.HandlerFunc {
	tools.DebugPrintF("[INFO] Starting Middleware %s", "Size")

	return RequestSizeLimiter(1024 * 1024 * 100)
}
